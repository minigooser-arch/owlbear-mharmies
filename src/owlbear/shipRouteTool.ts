...
export class ShipRouteToolController {
  ...
  undo(): ShipRouteKeyResult {
    ...
    this.sequence += 1;
    this.points.pop(); this.cells.pop(); this.stepCosts.pop(); this.facings.pop();
    this.currentPreview = undefined;
  ...
  clear(): ShipRouteKeyResult {
    if (!this.activation) return { action: "IGNORED" };
    this.sequence += 1;
    this.points = []; this.cells = []; this.stepCosts = []; this.facings = [];
    this.currentPreview = undefined;
    return { action: "EDITING" };
  }
  key(key: string): ShipRouteKeyResult {
    ...
    if (key === "Escape") { this.deactivate(); return { action: "CANCEL" }; }
  ...
  finish(): ShipRouteFinishResult {
    ...
    const result: ShipRouteFinishResult = {
      action: "COMMIT", shipId: active.shipId, startCell: { ...active.startCell },
      points: this.points.map((point) => ({ ...point })), cells: this.cells.map((cell) => ({ ...cell }))
    };
    this.deactivate();
    return result;
  }
  ...
  async click(point: Vector2): Promise<ShipRouteClickResult> {
    ...
    const sequence = ++this.sequence;
    const preview = await this.analyze(point);
    if (sequence !== this.sequence || this.activation?.shipId !== active.shipId) return { accepted: false, reason: "INACTIVE" };
    ...
    this.stepCosts.push(preview.stepCost ?? 0);
    this.facings.push(preview.nextFacing ?? this.facings.at(-1) ?? active.facing);
    this.currentPreview = undefined;
    return { accepted: true };
  }
  private deactivate(): void {
    this.activation = undefined; this.points = []; this.cells = []; this.stepCosts = []; this.facings = [];
    this.currentPreview = undefined; this.sequence += 1;
  }
  private async analyze(pointer: Vector2): Promise<ShipRoutePreview> {
    ...
    if (!cellSupportsDomain(scene, cell, "SEA")) return this.invalid(point, cell, "NON_NAVAL_TERRAIN", spent, remaining);
    const currentFacing = this.facings.at(-1) ?? active.facing;
    const requiredFacing = facingForStep(anchor, cell);
    if (!requiredFacing) return this.invalid(point, cell, "NOT_ORTHOGONAL", spent, remaining);
    const turnCost = quarterTurnCost(currentFacing, requiredFacing);
    ...
    return {
      ...
      spentMovementPoints: total, remainingMovementPoints: remaining - stepCost,
      stepCost, nextFacing: requiredFacing
    };
  }
  private invalid(point: Vector2, cell: GridCellCoord, reason: ShipRouteFailure, spent: number, remaining: number): ShipRoutePreview {
    return { point: { ...point }, cell: { ...cell }, valid: false, color: "#d32f2f", label: messageFor(reason), spentMovementPoints: spent, remainingMovementPoints: remaining, reason };
  }
}
