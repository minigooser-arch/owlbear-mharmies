...
function scene(): SceneState {
  return {
    ...
    states: [],
    relations: { red: { green: "ALLY", blue: "ENEMY" } },
    battleGroups: [],
    ...
    ships: {
      ...
      outsider: createRegisteredShip("blue", "BATTLESHIP", "WEST")
    },
    navalBattleRequests: [],
...
function snapshot(playerId: string, role: "GM" | "PLAYER" = "PLAYER") {
  ...
  return buildRoleSafeSnapshot({
    ...
    mapVisibleSourceIds: new Set(["enemy", "ally", "exited", "outsider"])
  });
}

describe("role-safe naval broadside targets", () => {
  it("gives the active side leader only living non-exited ships in the current battle", () => {
    ...
    expect(attacker?.broadsideTargets?.some((target) => target.id === "outsider")).toBe(false);
    expect(attacker?.broadsideTargets?.some((target) => target.id === "exited")).toBe(false);
  });
  ...
  it("gives the GM the same authoritative target set", () => {
