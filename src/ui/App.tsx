  ...
  return (
    <main className="app-shell" data-theme="letopis-wiki-light">
      ...
      <div className="content wiki-content">
        ...
        {tab === "CITIES" && isGM && <StrategicCityEditor role="GM" states={state.states} cities={state.strategicCities} onCreate={(city) => void state.sendStrategic({ type: "CREATE_STRATEGIC_CITY", city })} onUpdate={(cityId, patch) => void state.sendStrategic({ type: "UPDATE_STRATEGIC_CITY", cityId, patch })} onDelete={(cityId) => void state.sendStrategic({ type: "DELETE_STRATEGIC_CITY", cityId })} onOpenCellPicker={() => void state.send({ type: "OPEN_CITY_CELL_PICKER" })} onCloseCellPicker={() => void state.send({ type: "CLOSE_CITY_CELL_PICKER" })} pickedCells={state.cityCellPick?.cells ?? []} {...(state.cityCellPick ? { pickerSessionId: state.cityCellPick.sessionId } : {})} canPickCells={isGM && state.sceneReady} />}
        ...
        {tab === "MANAGEMENT" && isGM && <ManagementPage playerId={state.playerId} sides={state.sides} states={state.states} strategicCities={state.strategicCities} rebellionStatuses={state.rebellionStatuses} players={state.players} relations={state.relations} stateRelations={state.stateRelations ?? {}} settings={state.settings} leaderSideIds={state.leaderSideIds} onAction={send} runDiagnostic={state.runDiagnostic} />}
      </div>
