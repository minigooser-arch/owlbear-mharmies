...
it("shows one aggregated GM notice for multiple naval battle requests without changing the app grid", () => {
  ...
  expect(screen.getAllByRole("status", { name: "Заявки на морской бой" })).toHaveLength(1);
  const notice = screen.getByRole("status", { name: "Заявки на морской бой" });
  expect(notice).toHaveTextContent("Заявки на морской бой: 3");
